Building ravend for fun and profit
-----
...
Boost is downloaded from my Dropbox instead of sourceforge as in the original raven build, since sourceforge is slow and the download took 1 hour, dominating the total time of the build, for no real reason. If you don't like that, just remove the `boost_download_path` in `run_inside.sh` or change it to your own URL.
